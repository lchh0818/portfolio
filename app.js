...
if(document.body.dataset.page==='home'){
 ...
 if(experience){
   ...
   const gallery=[experience.image,...(experience.gallery||[])].filter(Boolean).map((image,index)=>`<img loading="lazy" src="${imageSource(image,'experiences/images')}" alt="${esc(experience.title)} 상세 사진 ${index+1}">`).join('');
   const video=experience.video?`<div class="experience-detail-video"><video controls preload="metadata" playsinline><source src="${imageSource(experience.video,'experiences/images')}" type="video/mp4">브라우저가 영상을 지원하지 않습니다.</video></div>`:'';
   document.querySelector('#detail').innerHTML=`<div class="detail-head"><p class="eyebrow">경험 · ${esc(experience.period)}</p><h1>${esc(experience.title)}</h1><p class="intro">${esc(experience.description)}</p></div>${video}${gallery?`<div class="experience-detail-media">${gallery}</div>`:''}<div class="detail-body experience-detail-body"><aside><strong>활동 기간</strong>${esc(experience.period)}<strong style="margin-top:24px">활동 분야</strong>경험 및 행사 운영</aside><article><h2>활동 상세</h2><p>${esc(experience.description)}</p></article></div>`;
 }else{
