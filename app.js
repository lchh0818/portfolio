...
const safe=s=>/^(https?:\/\/|\.\/|\/|assets\/|projects\/|experiences\/)/i.test(s)?esc(s):'';
const assetPath=(value,folder)=>{if(!value)return '';return /^(https?:\/\/|\.\/|\/|assets\/|projects\/|experiences\/)/i.test(value)?value:/^(projects|experiences)\//i.test(folder)?`${folder}/${value}`:`assets/${folder}/${value}`;};
...
const youtubeId=url=>{const match=String(url||'').match(/(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([\w-]{11})/i);return match?match[1]:'';};
const experienceVideos=experience=>[...(experience.videos||[]),...(experience.video?[{type:'local',src:experience.video}]:[])].map(video=>{if(video.type==='youtube'){const id=youtubeId(video.url);return id?`<div class="experience-detail-video"><iframe src="https://www.youtube.com/embed/${id}" title="${esc(video.title||experience.title+' 영상')}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>`:'';}const src=imageSource(video.src||video.url,'experiences/images');return src?`<div class="experience-detail-video"><video controls preload="metadata" playsinline><source src="${src}" type="video/mp4">브라우저가 영상을 지원하지 않습니다.</video></div>`:'';}).join('');
const tags=a=>a.map(s=>`<span class="tag">${esc(s)}</span>`).join('');
const cover=(p,detail=false)=>{const image=imageSource(p.image,`projects/${p.id}/images`);return `<div class="cover ${detail?'detail-cover':''}" style="--cover:${p.color};--cover-ink:${p.ink}">${image?`<img src="${image}" alt="${esc(p.name)} 프로젝트 이미지">`:`<span class="cover-meta">${esc(p.category)}</span><span class="cover-title">${esc(p.en)}</span><span class="cover-foot">${esc(p.name)}</span>`}</div>`;};
...
if(document.body.dataset.page==='home'){
 ...
 document.querySelector('#projects').insertAdjacentHTML('afterend',`<section id="role" class="role-section"><div class="section-head"><h2>Role</h2><small>Role</small></div><article class="role-card"><span class="role-period">Role</span><div class="role-list">${roleLines.map(role=>`<div class="role-line"><h3>${esc(roleTitle(role))}</h3>${rolePeriod(role)?`<span class="role-date">${esc(rolePeriod(role))}</span>`:''}</div>`).join('')}</div></article></section><section id="experiences" class="experiences-section"><div class="section-head"><h2>경험</h2><small>경험</small></div><div class="experience-list">${experiences.map(item=>`<a class="experience-card" href="experience.html?id=${encodeURIComponent(item.id)}" aria-label="${esc(item.title)} 경험 상세 보기"><span class="role-period">${esc(item.period)}</span><h3>${esc(item.title)}</h3><p>${esc(item.description)}</p></a>`).join('')}</div></section>`);
...
}else if(document.body.dataset.page==='experience'){
 const experience=(data.experiences||[]).find(item=>item.id===new URLSearchParams(location.search).get('id'));
 if(!experience){document.querySelector('main').innerHTML='<a class="back" href="index.html#experiences">← 경험 목록</a><h1>경험을 찾을 수 없습니다.</h1>';}
 else{
  document.title=experience.title+' | Development & Product Portfolio';
  document.querySelector('meta[name="description"]').content=experience.description;
  const gallery=[experience.image,...(experience.gallery||[])].filter(Boolean).map((image,index)=>`<img loading="lazy" src="${imageSource(image,'experiences/images')}" alt="${esc(experience.title)} 상세 사진 ${index+1}">`).join('');
   const details=(experience.details||[]).map(item=>`<section class="detail-extra"><h2>${esc(item.title)}</h2><p>${esc(item.text)}</p></section>`).join('');
   const links=[['Instagram',experience.instagram],['웹사이트',experience.website]].filter(([,url])=>/^https?:\/\//i.test(url||''));
   const linkMarkup=links.length?`<div class="detail-links">${links.map(([label,url])=>`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${label} ↗</a>`).join('')}</div>`:'';
   document.querySelector('#detail').innerHTML=`<div class="detail-head"><p class="eyebrow">경험 · ${esc(experience.period)}</p><h1>${esc(experience.title)}</h1><p class="intro">${esc(experience.description)}</p></div>${experienceVideos(experience)}${gallery?`<div class="experience-detail-media">${gallery}</div>`:''}<div class="detail-body experience-detail-body"><aside><strong>활동 기간</strong>${esc(experience.period)}<strong style="margin-top:24px">활동 분야</strong>경험 및 행사 운영${linkMarkup}</aside><article><h2>활동 상세</h2><p>${esc(experience.description)}</p>${details}</article></div>`;
 }
}else{
 const projects=[...(data.projects||[])].sort((a,b)=>(data.projectOrder||[]).indexOf(a.id)-(data.projectOrder||[]).indexOf(b.id));
 const params=new URLSearchParams(location.search);
 const experience=(data.experiences||[]).find(item=>item.id===params.get('experience'));
 if(experience){
    document.title=experience.title+' | Development & Product Portfolio';
    document.querySelector('meta[name="description"]').content=experience.description;
    document.querySelector('.back').href='index.html#experiences';
    document.querySelector('.back').textContent='← 경험 목록';
   const gallery=[experience.image,...(experience.gallery||[])].filter(Boolean).map((image,index)=>`<img loading="lazy" src="${imageSource(image,'experiences/images')}" alt="${esc(experience.title)} 상세 사진 ${index+1}">`).join('');
   document.querySelector('#detail').innerHTML=`<div class="detail-head"><p class="eyebrow">경험 · ${esc(experience.period)}</p><h1>${esc(experience.title)}</h1><p class="intro">${esc(experience.description)}</p></div>${experienceVideos(experience)}${gallery?`<div class="experience-detail-media">${gallery}</div>`:''}<div class="detail-body experience-detail-body"><aside><strong>활동 기간</strong>${esc(experience.period)}<strong style="margin-top:24px">활동 분야</strong>경험 및 행사 운영</aside><article><h2>활동 상세</h2><p>${esc(experience.description)}</p></article></div>`;
 }else{
    const p=projects.find(p=>p.id===params.get('id'));
    if(!p){document.querySelector('main').innerHTML='<a class="back" href="index.html#projects">← 프로젝트 목록</a><h1>프로젝트 또는 경험을 찾을 수 없습니다.</h1>';}
 ...
 const gallery=(p.gallery||[]).filter(Boolean).map((s,i)=>`<img loading="lazy" src="${imageSource(s,`projects/${p.id}/images`)}" alt="${esc(p.name)} 상세 이미지 ${i+1}">`).join('');
